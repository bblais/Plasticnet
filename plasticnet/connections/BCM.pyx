...
cdef class BCM_vonRossum2022(connection):
    cdef public double eta,tau,theta_o,gamma,yo,u
    cdef public np.ndarray theta
    cdef public np.ndarray phi
    cdef public np.ndarray initial_theta
    cdef public object initial_theta_range
    ...
    def __init__(self,neuron pre,neuron post,initial_weight_range=None,initial_theta_range=None):
        ...
        self.phi=pylab.zeros(self.post.N,float)        
        self.eta=1e-5
        self.tau=100.0
        self.gamma=0.0
        self.theta_o=1.0
        self.yo=0.0
        self.u=0
        self.name='BCM_vonRossum2022'
        self._reset()
    
        self.save_attrs.extend(['eta','tau','gamma','theta_o','yo','u'])
        self.save_data.extend(['theta','initial_theta','initial_theta_range'])
    ...
    cpdef update(self,double t,simulation sim):
        ...
        cdef double *W=<double *>self.weights.data
        cdef double *theta=<double *>self.theta.data
        cdef double *phi=<double *>self.phi.data
        cdef double *X
        cdef double *Y   # outputs for pre and post
        cdef int __wi
        cdef double eta=self.eta
        ...
        for __i in range(self.post.N):
            for __j in range(self.pre.N):
                ...
                phi[__i]=(Y[__i]-yo)*((Y[__i]-yo)-theta[__i])
                if phi[__i]<0:
                    W[__wi]+=sim.dt*(eta*(W[__wi]+u)*X[__j]*phi[__i]-eta*gamma*W[__wi])
                else:
                    W[__wi]+=sim.dt*(eta*X[__j]*phi[__i]-eta*gamma*W[__wi])
